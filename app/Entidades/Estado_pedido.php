...
class Estado_pedido extends Model
{
    ...
    public function obtenerPorId($idEstadoPedido)
    {
        $sql = "SELECT
                    idestadopedido,
                    nombre
                  FROM estado_pedidos WHERE idestadopedidos = $idEstadoPedido";
        $lstRetorno = DB::select($sql);
    ...
    {
        $sql = "DELETE FROM estado_pedidos WHERE idestadopedidos=?";
        $affected = DB::delete($sql, [$this->idestadopedidos]);
    }

    public function insertar()
    {
        $sql = "INSERT INTO estado_pedidos (nombre) VALUES (?);";
